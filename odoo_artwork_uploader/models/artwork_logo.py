...
class ArtworkLogo(models.Model):
    ...
    def get_logo_data(self):
        """Get logo data as dict for API responses"""
        self.ensure_one()
        ...
        if self.color_data:
            try:
                color_data = json.loads(self.color_data)
        ...
        return {
            ...
            'isRaster': self.is_raster,
            'hasMixedContent': self.has_mixed_content,
            'colorCount': self.color_count,
            'hasCMYK': self.has_cmyk,
            'hasRGB': self.has_rgb,
            'hasPantone': self.has_pantone,
            'isCMYKPreserved': self.is_cmyk_preserved,
            'widthPx': self.width_px,
            'heightPx': self.height_px,
            'widthMm': self.width_mm,
            'heightMm': self.height_mm,
            'hasFonts': self.has_fonts,
            'minStrokeWidth': self.min_stroke_width,
            'hasTransparency': self.has_transparency,
            'colorData': color_data,
        }

    @api.model
    def create_from_upload(self, project_id, file_data, filename):
        """Create logo from uploaded file data"""
        ...
        file_type = type_map.get(ext, False)
        if not file_type:
            raise UserError(f"Unsupported file type: {ext}")
        ...
        logo_vals = {
            ...
            'file_type': file_type,
            'is_vector': file_type in ['svg', 'pdf', 'ai', 'eps'],
            'is_raster': file_type in ['png', 'jpeg'],
        }
        
        return self.create(logo_vals)
