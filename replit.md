...
- **Canvas System**: Interactive workspace for logo manipulation with real-time property editing, including multi-select, group move, resize, and rotation. Dual-canvas mode for applique templates: Canvas 1 (Badge Artwork) and Canvas 2 (Embroidery Artwork) with tab switching, "Embroider These Elements" move action, element count badges, and canvas-scoped selection. Controlled by `canvasIndex` field on canvas_elements (0=badge, 1=embroidery). Fully sandboxed - only activates for applique template IDs.
