...
- **CRITICAL MIGRATION BLOCKER IDENTIFIED & SOLUTION DOCUMENTED** (October 7, 2025): Discovered PyMuPDF cannot replicate Ghostscript's precise PDF bounds extraction (30-50mm deviations observed in testing). Ghostscript `-sDEVICE=bbox` provides millimeter-accurate tight content bounds essential for artwork positioning, while PyMuPDF returns page size or inaccurate content bounds. Odoo.sh managed hosting prohibits apt-installing Ghostscript. Created dual-track solution: (1) ODOO_SH_GHOSTSCRIPT_REQUEST.md - comprehensive support ticket to request Ghostscript addition to Odoo.sh base image, (2) GHOSTSCRIPT_MICROSERVICE_GUIDE.md - complete implementation guide for external FastAPI microservice (deployable FREE on Render/Fly.io) providing Ghostscript bbox extraction via HTTP API. Microservice approach preserves exact accuracy while working within Odoo.sh constraints, can be retired if Ghostscript becomes available natively.
- **Odoo Migration Documentation & AI Vectorization Removal** (October 7, 2025): Removed automatic AI vectorization feature (VectorizerModal) to prepare for Odoo migration - kept manual vectorization service form for design team submissions. Created comprehensive migration documentation: ODOO_MIGRATION_PLAN.md (overall strategy), FRONTEND_BUILD_GUIDE.md (React to Odoo build pipeline), PYTHON_CONTROLLERS_GUIDE.md (Express to Odoo controllers), PDF_SVG_PROCESSING_GUIDE.md (TypeScript to Python processing), IMPLEMENTATION_GUIDE.md (14-day implementation plan), MIGRATION_SUMMARY.md (executive summary). Migration targets: $0 hosting costs (vs $20-100/month Replit), 1000+ concurrent users (vs 50-100 limit), native cart integration, 2-week timeline.
- **Complex Vector Detection & PNG Fallback System**: Implemented path-count-based complexity detection (>4000 paths or >5000 total elements) with automatic PNG fallback generation for canvas display. System analyzes SVG files during upload, counts vector paths and elements, generates high-resolution PNG (300 DPI) for canvas rendering when complexity threshold exceeded, while preserving original SVG for PDF output. Prevents browser crashes from rendering thousands of DOM elements while maintaining perfect vector quality in final PDFs. Database tracks complexity metrics (pathCount, elementCount, detectedAt) and fallback filenames. Frontend automatically uses PNG for canvas display, SVG for color overrides/recoloring. Fixes critical issue where 6.6MB files with 47,000+ paths caused complete browser hangs (October 4, 2025).
- **Large SVG Performance Fix** (DEPRECATED - Replaced by path-count detection): Previous file-size-based threshold (>1MB) replaced with accurate path-count analysis. Old system incorrectly triggered on file size rather than actual rendering complexity (October 4, 2025).
...
- **Vectorization Services**: Raster file detection with photographic approval and manual professional vectorization service request form for design team submissions. Automatic AI vectorization removed in preparation for Odoo migration (October 7, 2025).
