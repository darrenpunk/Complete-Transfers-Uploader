...
- **CRITICAL: Painted Bounds Cannot Detect White Elements**: Ghostscript's bbox device only detects visible ink/color on the page. White logos, text, or graphics are completely invisible to painted bounds detection and will be incorrectly cropped out. This is a fundamental limitation of raster-based bounds analysis. Clipping path detection also unreliable (often finds wrong vectors). **Solution**: Use full viewBox dimensions for PDFs - do NOT auto-fit bounds for PDFs as it will crop white content.
