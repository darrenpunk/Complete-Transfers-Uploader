...
**Vectorizer Small Detail Enhancement (2025-08-03)**: Enhanced vectorizer.ai API parameters to better preserve small details like dots. Added `processing.despeckle_tol=0.1` (low tolerance to keep small dots), `processing.curve_fitting=true` (shape fitting for dots/circles), and `processing.corner_threshold=60` (sensitive corner detection). Also improved stroke removal logic to detect and preserve small elements by converting them to fills instead of removing them completely.
...
**White Background Removal Fix (2025-08-03)**: Fixed issue where white backgrounds were automatically removed from JPEG files during AI vectorization. The automatic removal was happening in the backend `convertVectorizedSvgToFullCmyk` function. Solution: Added a user-controlled toggle checkbox "Remove White Background" in the vectorizer modal that allows users to choose whether to remove white backgrounds. The toggle is off by default, preserving the original image's white areas unless explicitly requested by the user.
...
**PDF Filename Display Fix (2025-08-04)**: Fixed critical issue where PDF files containing raster content showed ".pdf" instead of ".png" in the vectorizer modal. The problem was that PNG extraction wasn't happening before the vectorizer modal opened. Solution: Modified the handleVectorizeWithAI function in upload-tool.tsx to properly extract PNG files from PDFs using the /api/logos/:id/raster-image endpoint, create proper File objects with correct .png filenames, and update the pendingRasterFile state before opening the vectorizer modal. Now when users upload PDFs with embedded raster content and click "Vectorize with AI", the system correctly extracts the PNG and displays "document.png" instead of "document.pdf" in the vectorizer interface.
...
**Vectorizer Small Detail Preservation Fix (2025-08-03)**: Fixed issue where small details like dots in letters (e.g., dot in "i" of "FRIENDLY") were being removed during stroke cleanup. Problem: The stroke removal function was too aggressive, removing all stroke-only elements including small details. Solution: Enhanced the `removeVectorizedBackgroundsRegex` function to detect and preserve small elements (circles < 10px radius, rectangles < 20px, short paths < 100 chars). Small stroke-only elements are now converted to filled elements instead of being removed. Added comprehensive logging to track element counts and preservation. This ensures critical typography details are maintained while still removing unwanted strokes.

**Vectorizer Letter "I" Missing Fix (2025-08-03)**: Fixed critical issue where the letter "I" in "FRIENDLY" was missing from vectorized output. Problem: The vectorizer.ai API was creating the letter "I" as a stroke-only element, which was then removed by our aggressive stroke cleanup. Solution: Enhanced detection logic to specifically look for narrow vertical elements (width < 20px, height > 20px for rectangles; width < 15px, height > 20px for paths). These elements are now preserved and converted to filled elements. Also added vectorizer API parameters to better preserve text: `processing.smoothing=0` (disable smoothing), `processing.group_by=none` (preserve individual letters), and improved min_area/simplify_tolerance settings.
