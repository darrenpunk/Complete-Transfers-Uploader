...
export function calculateSVGContentBounds(svgContent: string): { width: number; height: number; minX: number; minY: number; maxX: number; maxY: number } | null {
  try {
    ...
    // CRITICAL FIX: Process <use> elements that reference <defs>
    // Many SVGs (especially from PDF conversion) use <defs> + <use> for text glyphs
    console.log('🔍 DEBUG: Processing <use> elements...');
    ...
    while ((useMatch = useRegex.exec(svgContent)) !== null) {
      ...
      console.log(`🔍 Found <use> referencing #${refId}`);
      
      // Extract transform from <use> element
      const transformMatch = useElement.match(/transform="([^"]+)"/);
      let translateX = 0, translateY = 0;
      
      if (transformMatch) {
        const translateMatch = transformMatch[1].match(/translate\(([-\d.]+)[\s,]+([-\d.]+)\)/);
        if (translateMatch) {
          translateX = parseFloat(translateMatch[1]);
          translateY = parseFloat(translateMatch[2]);
          console.log(`🔍 <use> has transform: translate(${translateX}, ${translateY})`);
        }
      }
      
      // Find the referenced element in <defs>
      const defsPattern = new RegExp(`<(?:path|g)[^>]*id="${refId}"[^>]*(?:d="([^"]*)")?[^>]*>`, 'g');
      const defMatch = defsPattern.exec(svgContent);
      
      if (defMatch && defMatch[1]) {
        // Found a path with d attribute
        const pathData = defMatch[1];
        console.log(`🔍 Resolved <use> to path: ${pathData.substring(0, 80)}...`);
        
        // Extract coordinates from the referenced path and apply transform
        coordinateRegex.lastIndex = 0;
        let coordMatch;
        while ((coordMatch = coordinateRegex.exec(pathData)) !== null) {
          if (coordMatch[1] && coordMatch[2]) {
            const x = parseFloat(coordMatch[1]) + translateX;
            const y = parseFloat(coordMatch[2]) + translateY;
            updateBounds(x, y);
          } else if (coordMatch[4]) {
            const coords = coordMatch[4].split(/[,\s]+/).map(c => parseFloat(c)).filter(c => !isNaN(c));
            for (let i = 0; i < coords.length; i += 2) {
              if (i + 1 < coords.length) {
                updateBounds(coords[i] + translateX, coords[i + 1] + translateY);
              }
            }
          }
        }
    ...
    console.log(`📍 Processed ${useCount} <use> elements`);
    if (useCount > 0 && hasCoordinates) {
      console.log(`✅ Successfully resolved <use> elements to content bounds: ${minX.toFixed(1)},${minY.toFixed(1)} → ${maxX.toFixed(1)},${maxY.toFixed(1)}`);
    }
